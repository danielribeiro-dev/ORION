...
class Role(Enum):
    ...
    POWER_USER = "power_user" # Can execute L2 with supervision
    USER = "user"             # Can execute L1, L2 needs approval
    GUEST = "guest"           # Limited to L0/L1
...
class AuthService:
    ...
    def create_user(self, username: str, password: str, role: Role) -> bool:
        ...
        self._save_users()
        logger.info(f"[Auth] User '{username}' created with role '{role.value}'.")
        return True
